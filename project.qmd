...
```{r}
# downloads BLS QCEW Data
#for(x in 2010:2023){
#  qcew_data(x)
#  qcew_processed_data(x)
#  qcew_check(x)
#}
...
fed_funds <- fed_funds |> 
  ...
  mutate(# level lags
         ...
         fed_funds_chg_lag1 = lag(fed_funds_chg_qtr, 1),
         fed_funds_chg_lag2 = lag(fed_funds_chg_qtr, 2),
         fed_funds_chg_lag3 = lag(fed_funds_chg_qtr, 3),
         fed_funds_chg_lag4 = lag(fed_funds_chg_qtr, 4)
         ) 
...
```{r}
fed_funds_graph <- qcew_panel_district_data |> 
  group_by(date) |> 
  select(date, fed_funds_rate) |> 
  distinct() |> 
  ggplot(mapping = aes(x = date, y = fed_funds_rate)) +
  geom_line() +
  labs(title = "Fed Funds Rate over Time", x = "Time", y = "Fed Funds Rate")

emplvl_growth <- qcew_panel_district_data |> 
  group_by(date) |> 
  summarise(employment = sum(avg_emplvl)) |> 
  mutate(log_employment = log(employment),
         employment_growth = log_employment - lag(log_employment, 1)) |> 
  ggplot(mapping = aes(x = date, y = employment_growth)) +
  geom_line() +
  labs(title = "National Change in Employment", x = "Time", y = "Change in Employment")

emplvl_growth_fed_district <- qcew_panel_district_data |> 
  group_by(fed_district, date) |> 
  summarise(employment = sum(avg_emplvl)) |> 
  mutate(log_employment = log(employment),
         employment_growth = log_employment - lag(log_employment, 1)) |> 
  ggplot(mapping = aes(x = date, y = employment_growth)) +
  geom_line() +
  facet_wrap(~fed_district) +
  labs(title = "Change in Employment per Fed District", x = "Time", y = "Change in Employment")


wage_growth_fed_district <- qcew_panel_district_data |> 
  group_by(fed_district, date) |> 
  summarise(wage = weighted.mean(avg_wkly_wage, w = avg_emplvl)) |> 
  mutate(log_wage = log(wage),
         wage_growth = log_wage - lag(log_wage, 1)) |> 
  ggplot(mapping = aes(x = date, y = wage_growth)) +
  geom_line() +
  facet_wrap(~fed_district) +
  labs(title = "Average Wage Growth per Fed District", x = "Time", y = "Average Wage Growth")

district_summary <- qcew_panel_district_data |> 
  group_by(fed_district) |> 
  summarise(counties = n_distinct(area_fips),
            avg_employment = mean(avg_emplvl),
            avg_weekly_wage = mean(avg_wkly_wage),
            avg_employment_growth = mean(emplvl_growth_yr, na.rm = TRUE),
            avg_wage_growth = mean(wage_growth_yr, na.rm = TRUE)) |> 
  kable()

fed_funds_graph
emplvl_growth
emplvl_growth_fed_district
wage_growth_fed_district
district_summary

ggsave("output/figures/fed_funds.pdf",
       plot = fed_funds_graph,
       width = 12,
       height = 8)

ggsave("output/figures/employment_growth_national.pdf",
       plot = emplvl_growth,
       height = 12,
       width = 8)

ggsave("output/figures/employment_growth_fed_district.pdf",
       plot = emplvl_growth_fed_district,
       width = 12,
       height = 8)

ggsave("output/figures/wage_growth_fed_district.pdf",
       plot = wage_growth_fed_district,
       width = 12,
       heigh = 8)
...
```{r}
data <- read.csv("data/processed/analysis_panel.csv")

data <- data |> 
  mutate(date = ymd(as_datetime(date)))

emplvl_lm <- feols(emplvl_growth_yr ~ i(fed_district, fed_funds_chg_qtr) | 
                     area_fips + date, data = data)

emplvl_lm1 <- feols(emplvl_growth_yr ~ i(fed_district, fed_funds_chg_lag1, ref = "St. Louis") + 
                      i(fed_district, fed_funds_chg_qtr, ref = "St. Louis") | 
                      area_fips + date, data = data)
  

emplvl_lm2 <- feols(emplvl_growth_yr ~ i(fed_district, fed_funds_chg_qtr, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag1, ref = "St. Louis") 
                    + i(fed_district, fed_funds_chg_lag2, ref = "St. Louis") |
                      area_fips + date, data = data)

emplvl_lm3 <- feols(emplvl_growth_yr ~ i(fed_district, fed_funds_chg_qtr, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag1, ref = "St. Louis") 
                    + i(fed_district, fed_funds_chg_lag2, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag3, ref = "St. Louis") |
                      area_fips + date, data = data)
  
emplvl_lm4 <- feols(emplvl_growth_yr ~ i(fed_district, fed_funds_chg_qtr, ref = "St. Louis") 
                    + i(fed_district, fed_funds_chg_lag1, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag2, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag3, ref = "St. Louis")
                    + i(fed_district, fed_funds_chg_lag4, ref = "St. Louis") |
                      area_fips + date, data = data)


etable(emplvl_lm, emplvl_lm1, emplvl_lm2, emplvl_lm3, emplvl_lm4, vcov = ~ fed_district + date) |> 
  kable()
``` 
